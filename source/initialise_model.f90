...
subroutine Init_Density_Prof(ind_z_max, ind_z_surf, DZ_max, rho0, rhoi, R, Ec, Ec2, Eg, g, kg, acav, tsav, DZ, Rho, M, rgrain2)
    !*** Initialise the density, mass and grain size profile ***!
        
    ! declare arguments
    integer, intent(in) :: ind_z_max, ind_z_surf    
    double precision, intent(in) :: DZ_max, rho0, rhoi, R, Ec, Ec2, Eg, g, kg, acav, tsav
    double precision, dimension(ind_z_max), intent(inout) :: DZ, Rho, rgrain2
    double precision, dimension(ind_z_max), intent(out) :: M

    ! declare local variables
    integer :: ind_z
    double precision :: a, Rho_t, A1, A2, P, Ps, Ptot, dRho_part1, dRho_part2, dRho, theta, w1, w2, dt, rgrain
    ...
    rgrain2(ind_z_surf) = (100.E-3/rho0)**2.

    P = 0.                      ! load per unit area

    do ind_z = (ind_z_surf-1), 1, -1

        dt = Rho(ind_z+1)*DZ(ind_z+1)/(acav/(365.25*24.*3600.))

        ! Calculate grain size
        rgrain2(ind_z) = rgrain2(ind_z+1) + kg*exp(-Eg/(R*tsav))*dt
        rgrain = rgrain2(ind_z)**0.5       ! grain size in m

        ! Calculate the driving stress
        P = P + g*M(ind_z+1)  ! load per unit area
        Ps = rhoi/Rho(ind_z+1)*P          ! grain-load stress: correction for the fact that only grains, not voids, support the load
        Ptot = (rhoi/Rho(ind_z+1)-1)*Ps   ! effective stress
    
        ! Calculate the components of the densification rate
        dRho_part1 = A1*exp(-Ec/(R*tsav))*Ptot/(rgrain**2.)
        dRho_part2 = A2*exp(-Ec2/(R*tsav))*Ptot**1.8/(rgrain**1.4)
    
        ! Calculate weighting factors
        theta = 1./(1.+exp(-a*(Rho(ind_z+1) - Rho_t)))
        w1 = 1. - theta
        w2 = theta
    
        ! Calculate the total densification
        dRho = (w1*dRho_part1 + w2*dRho_part2) * Rho(ind_z+1) * dt
    
        ! Update density
        Rho(ind_z) = Rho(ind_z+1) + dRho
    ...
    print *, 'Initial grain size 10 lowermost layers (in mm):'
    print *, 1000.*(rgrain2(1:10)**0.5)
    print *, " "
...
subroutine Init_Temp_Prof(ind_z_max, ind_z_surf, beginT, tsav, pi, T, Rho, Depth, rhoi)
    ...
    do ind_z = ind_z_surf, 1, -1                                ! temperature - depth loop
        ...
        if (ind_z == 1) then
            ...
            if (T(ind_z) > 273.15) T(ind_z) = 273.15
        endif
