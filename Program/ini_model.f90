...
!---------------------------------
	...
	do a = 1,numTimes-1
	  do  b = 1,numSteps
        ...
        !Rho0FM(step) = 97.49 + 0.769*TempFM(step) + 4.49*ff10FM(step) ! Helsen
        !Rho0FM(step) = 481. + 4.834*(TempFM(step) - 273.15) ! Kuipers Munneke (2015)
        !Rho0FM(step) = 315.
	  end do
	end do
    ...
    do step = 1,numPoints
        !PSolFM(step) = PSolFM(step)*1.15
        !PliqFM(step) = PliqFM(step)*1.15
        !SublFM(step) = SublFM(step)*1.15
        !DrifFM(step) = DrifFM(step)*1.15
        MeltFM(step) = MeltFM(step)*1.15
    end do
...
!----------------------------
	...
	! After Reeh, 2008	
!	if (AveMelt(lon,lat) .lt. AveAcc(lon,lat)) then
!	  tsav = AveTsurf(lon,lat) + 29.008 * AveMelt(lon,lat)/1000.
!	else
!	  tsav = AveTsurf(lon,lat) + 29.008 * AveAcc(lon,lat)/1000.
!	endif
...
!----------------------------- 	
    ...
    do k=kUL-1,1,-1
        ...
        if (Rho(k+1) .le. 550.) then
            if (trim(domain) .eq. "FGRN11" .or. trim(domain) .eq. "FGRN055") then
                !cons = 1.042 - 0.0916*log(acav)
                !cons = 1.
                !cons = 0.6545 + 0.0022*log(acav)    ! FGRN055 new cons
                !cons = 0.6569 + 0.0067*log(acav)    ! FGRN055 new cons, new fit 1957-2020 run
                !cons = 0.3867 + 0.0415*log(acav)    ! FGRN055 new cons, new fit 1957-2020 run, rho=315
            ...
            if (trim(domain) .eq. "FGRN11" .or. trim(domain) .eq. "FGRN055") then
                !cons = 1.734 - 0.2093*log(acav)
                !cons = 1.6942 - 0.1941*log(acav)    ! FGRN055 new cons
                !cons = 1.
                !cons = 1.7243 - 0.2011*log(acav)    ! FGRN055 new cons, new fit 1957-2020 run
                !cons = 1.7231 - 0.2009*log(acav)    ! FGRN055 new cons, new fit 1957-2020 run, rho=315
